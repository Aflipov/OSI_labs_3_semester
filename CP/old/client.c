...
void request_turn_order(void *socket) {
  Message req = {0};
  req.type = MSG_TURN_ORDER;
  strncpy(req.sender, player_login, MAX_PLAYER_NAME - 1);
  strncpy(req.recipient, "SERVER", MAX_PLAYER_NAME - 1);

  send_message(socket, &req);

  Message resp = {0};
  int size = zmq_recv(socket, &resp, sizeof(Message), 0);
  if (size != sizeof(Message)) {
    printf("Failed to receive game state response\n");
    return;
  }

  if (resp.type == MSG_TURN_ORDER) {
    printf("%s\n", resp.data);

    if (strstr(resp.data, "Your turn")) {
      my_turn = true;
    } else {
      my_turn = false;
    }
...
void handle_server_response(void *socket, Message *msg) {
  if (msg->type == MSG_SHOT_RESULT) {
    ...
    if (strcmp(msg->data, "Opponent is getting ready...") == 0) {
      return;
    } else if (strcmp(msg->data, "Your turn!") == 0) {
      my_turn = true;
    } else {
      my_turn = false;
    }
