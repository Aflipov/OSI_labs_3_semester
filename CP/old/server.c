...
void handle_register(Message *msg, void *socket) {
  if (find_player(msg->sender) != NULL) {
    ...
    strncpy(response.data, "Player already registered", MAX_MESSAGE_SIZE - 1);
  ...
  if (player_count >= 100) {
    ...
    strncpy(response.sender, "SERVER", MAX_PLAYER_NAME - 1);
    strncpy(response.recipient, msg->sender, MAX_PLAYER_NAME - 1);
    strncpy(response.data, "Server is full", MAX_MESSAGE_SIZE - 1);
    send_message(socket, &response);
    return;
  }

  Player *player = &players[player_count];
  strncpy(player->login, msg->sender, MAX_PLAYER_NAME - 1);
  player->login[MAX_PLAYER_NAME - 1] = '\0';
  ...
  strncpy(response.sender, "SERVER", MAX_PLAYER_NAME - 1);
  strncpy(response.recipient, msg->sender, MAX_PLAYER_NAME - 1);
  strncpy(response.data, "Registered successfully", MAX_MESSAGE_SIZE - 1);
  send_message(socket, &response);

  printf("Player %s registered\n", player->login);
}

// Обработка создания игры
void handle_create_game(Message *msg) {
...
void handle_join_game(Message *msg) {
  Player *player = find_player(msg->sender);
  if (player == NULL) {
    Message response = {0};
    response.type = MSG_ERROR;
...
// Обработка приглашения игрока
void handle_invite_player(Message *msg) {
  ...
  if (inviter == NULL || !inviter->in_game) {
    ...
    send_message(inviter->socket, &response);
    return;
  }
  ...
  Message invite = {0};
  invite.type = MSG_INVITE_PLAYER;
  strncpy(invite.sender, msg->sender, MAX_PLAYER_NAME - 1);
  strncpy(invite.recipient, msg->recipient, MAX_PLAYER_NAME - 1);
  strncpy(invite.game_name, game->name, MAX_GAME_NAME - 1);
  snprintf(invite.data, MAX_MESSAGE_SIZE, "You are invited to game '%s' by %s",
           game->name, msg->sender);
  send_message(invitee->socket, &invite);
...
void handle_place_ship(Message *msg) {
  Player *player = find_player(msg->sender);
...
void handle_game_state(Message *msg) {
  ...
  if (both_ready && game->player_count == MAX_PLAYERS) {
    ...
    //   Player *p = find_player(game->players[i]);
    //   if (p != NULL) {
    //     notify.type = MSG_GAME_STATE;
    //     strncpy(notify.sender, "SERVER", MAX_PLAYER_NAME - 1);
    //     strncpy(notify.recipient, game->players[i], MAX_PLAYER_NAME - 1);
    //     if (i == game->current_turn) {
    //       strncpy(notify.data, "Your turn!", MAX_MESSAGE_SIZE - 1);
    //       printf("%s's turn\n", game->players[i]);
    //     } else {
    //       strncpy(notify.data, "Opponent's turn", MAX_MESSAGE_SIZE - 1);
    //     }
    //     send_message(p->socket, &notify);
    //   }
    // }

    notify.type = MSG_GAME_STATE;
    strncpy(notify.sender, "SERVER", MAX_PLAYER_NAME - 1);
    strncpy(notify.recipient, player->login, MAX_PLAYER_NAME - 1);
    ...
    send_message(player->socket, &notify);
  }
    ...
    strncpy(notify.sender, "SERVER", MAX_PLAYER_NAME - 1);
    strncpy(notify.recipient, player->login, MAX_PLAYER_NAME - 1);
    strncpy(notify.data, "Opponent is getting ready...", MAX_MESSAGE_SIZE - 1);
    send_message(player->socket, &notify);
  }
}

// Обработка выстрела
void handle_make_shot(Message *msg) {
  Player *player = find_player(msg->sender);
  if (player == NULL || !player->in_game) {
    return;
  }
  ...
  // Отправка результата стреляющему игроку
  Message response = {0};
  ...
  if (opponent != NULL) {
    Message notify = {0};
    notify.type = MSG_SHOT_RESULT;
    notify.x = x;
    notify.y = y;
    notify.shot_result = result;
    strncpy(notify.sender, "SERVER", MAX_PLAYER_NAME - 1);
    strncpy(notify.recipient, game->players[opponent_idx], MAX_PLAYER_NAME - 1);

    if (result == SHOT_MISS) {
      strncpy(notify.data, "Opponent missed", MAX_MESSAGE_SIZE - 1);
    } else {
      snprintf(notify.data, MAX_MESSAGE_SIZE, "Opponent hit at (%d,%d)", x, y);
    }

    send_message(opponent->socket, &notify);
  }
  ...
  if (check_game_over(game->boards[opponent_idx])) {
    ...
    for (int i = 0; i < game->player_count; i++) {
      ...
      if (p != NULL) {
        ...
        send_message(p->socket, &game_over);
...
void handle_list_games(Message *msg) {
  Player *player = find_player(msg->sender);
  if (player == NULL) {
    return;
  }
...
int main() {
  ...
  while (1) {
    ...
    // Обновляем сокет игрока при регистрации или если игрок уже зарегистрирован
    Player *player = find_player(msg.sender);
    if (player != NULL) {
      player->socket = socket;
    }
    ...
    case MSG_CREATE_GAME:
      handle_create_game(&msg);
      break;
    case MSG_JOIN_GAME:
      handle_join_game(&msg);
      break;
    case MSG_INVITE_PLAYER:
      handle_invite_player(&msg);
      break;
    case MSG_PLACE_SHIP:
    ...
    case MSG_GAME_STATE:
      ...
      break;
    case MSG_MAKE_SHOT:
      handle_make_shot(&msg);
      break;
    case MSG_LIST_GAMES:
      handle_list_games(&msg);
      break;
    default:
